...
A hyperlocal activity board for your city (it starts in the San Francisco Bay Area, and more cities can be added). Post a sports or social meetup, and others RSVP.
...
| `018_rename_message` | updates the app's name in one database error message |
| `019_retention_features` | recurring meetups, "bring a friend", "still coming?" and the waitlist, notification settings and quiet hours, the server-only send log, and host stats |
| `020_cities_and_activities` | which city a meetup is in (everything that exists becomes the Bay Area), the name of an "Other" activity, and the city a person browses |
...
SoFly emails people when: someone asks to join their approval-only meetup, a host answers a request, a meetup they were going to is cancelled or changed, the day before a meetup they're part of (and "Still coming?" the morning of), the day after (a one-tap "how was it?"), a friend they invited joins, a spot opens on a waitlist, and a weekly note of meetups they might like. Each person can switch kinds off, or all email, on the Me tab. Login codes are separate and always sent.
...
4. Redeploy. The daily job (`vercel.json`) then runs at 9am Pacific, sending reminders, "Still coming?" prompts, feedback requests, host wrap-ups and (on Thursdays) the weekly digest. In Vercel, **Settings → Cron Jobs** lists it and has a **Run** button to test it.
...
- **Keeping people coming back (migration 019).** Seven features work together, each built so the app keeps working if the migration hasn't been run yet (the new parts simply don't appear):
  - **Recurring meetups.** The post form has a **Repeat** option (every week or every 2 weeks, 2 to 12 meetups). Each date is an ordinary meetup with its own guest list, at the same Pacific wall-clock time across clock changes. The feed shows a series once (its next date) unless you're browsing by date; the meetup page lists the other dates, and the host can **Add more dates** or **Cancel this and all N remaining** (everyone who joined is told). **Post again** copies one of your own past meetups with the date moved forward, and Me nudges when a series has run out of dates.
  - **Bring a friend.** After joining, a card shares a link straight to the meetup. The link carries an HMAC-signed token tied to that meetup and the inviter (from `INVITE_SECRET` if you set it, otherwise derived from the service key), so it can't be forged, edited or reused, and nobody can use it to find out who's going to a private meetup. The friend sees "Ann invited you"; the database records who invited them only if the inviter really is the host or an approved guest; hosts see "Invited by Ann"; the inviter is told once when their friend is in.
  - **Notification settings.** On Me you can switch off *reminders*, *new meetups for you* and *activity on my meetups* separately, and set quiet hours (Pacific time, for push only). Answers to your requests, cancellations and changes to a meetup you joined always come through. Nudges are limited to 3 push notifications a day, and a server-only send log makes the daily job safe to run twice. Right after someone joins a meetup, a one-time prompt offers notifications (never on arrival, never again after "Not now").
  - **Weekly digest.** On Thursdays, people who picked interests get a short note of meetups matching them: never full, women-only/men-only, age-limited-for-them, already-joined or hosted ones, never empty, never twice in 6 days.
  - **Fewer no-shows.** The morning of a meetup, guests who haven't confirmed are asked "Still coming?" (a card on the meetup page too). "I can't make it" frees the spot: people on the waitlist (for full, open meetups) are told, first come first served, and the host is told about a drop within 48 hours. Adding spots also tells the waitlist.
  - **After a meetup.** The "How was it?" message carries "you met N people" and up to three similar meetups; answering in the app shows them straight away. The host gets a wrap-up the next morning (how many came, how many had been before) with a nudge to post the next one.
  - **Host recognition and milestones.** Guests see how many people came back to a host's meetups; hosts get a summary on Me. Members see getting-started steps, then "Your journey" (first meetup, Regular, Explorer, Around the Bay, Host, Connector...), all worked out from what really happened and stored nowhere.
- **Cities and "Other" activities (migration 020).** SoFly is built for more than one city but runs in one today, the San Francisco Bay Area, and with one city nothing about how it looks or works changes: no city picker appears anywhere.
  - **Opening a new city.** Run migration 020, then add the city to `CITIES` in `lib/cities.ts` (an id that never changes, a name, and its neighborhoods in groups), and give each new neighborhood a scene in `lib/cover.ts` (a test fails until you do). As soon as there are two cities, a city picker appears at the top of the feed, on Me ("Your city") and in the post form; the feed, the weekly digest and "similar meetups" then only show meetups from the person's own city; and a neighborhood must belong to its meetup's city. Everything that exists before then is placed in the Bay Area by the migration.
  - **Time zones.** Every time in SoFly (posting, reminders, the 9am daily job, quiet hours, repeating dates) is Pacific. A city in another time zone needs those made per-city first; a test stops a non-Pacific city being added by accident.
  - **"Other" activities.** The category list has "Other sports & fitness" and "Other social & interests". Choosing one asks the host "What is it?" (up to 40 characters, e.g. "Frisbee golf"), and that name is what shows on the card, the meetup page and the share preview. The feed filter and interests use the two categories.
- **The name and logo.** The app is called SoFly, with a butterfly as its icon (drawn in code in `lib/butterfly.ts`, so it stays sharp and uses the palette; it's the favicon, the home-screen icon, the header logo (the very same deep-green tile) and part of the link previews). The web address is `so-fly-ai.vercel.app` (the old address redirects to it). Set `NEXT_PUBLIC_SITE_URL=https://so-fly-ai.vercel.app` in Vercel and redeploy, so emails and link previews use it (don't rely on Vercel's auto-detection: it picks the shortest domain, which may be the old one). In Supabase (Authentication → URL Configuration) set the Site URL to the new address and add `https://so-fly-ai.vercel.app/**` to the redirect URLs. A few things still carry the old name because they live outside the code: the GitHub repository name, the Gmail account used for contact and sending emails, and two settings inside Supabase: the login-code email template and the SMTP sender name (see "Email templates" and "Custom SMTP" above; set both to SoFly). Icons are cached by browsers and phones for a year, so they carry a version (`ICON_REV` in `lib/brand.ts`): bump it whenever the icon changes.
...
- **Migrations 010-020 can be run again safely.** If an earlier run stopped partway (for example with "already exists"), just run the file once more and it finishes the job.
