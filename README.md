...
- **Editing a meetup.** Hosts get an **Edit** button at the top of their meetup's page (until it starts or is cancelled) to change the date and time, neighborhood, venue and address. (The number of spots is changed further down, under "Manage this meetup".) Everything is saved in one step, so a meetup can't end up with its new time but its old place; for approval-only meetups the private address stays private. Once saved, everyone who had joined gets an email and a push notification saying what changed from and to (people whose request is still pending or was declined aren't told, and the push never shows the address on a lock screen). Someone who turned emails off still gets the push, and vice versa. Needs migration 017.
